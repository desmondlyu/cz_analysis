<!-- ================================================================
  ...
  最後更新：2026-06-10，Session 1daf13ca
  ================================================================ -->
...
- 前端為 React 應用程式，使用 Vite 建置，並將所有的 JS、CSS 直接內聯打包進單一 HTML 檔案（即 `index.html`），以供地端離線啟動與分析。

### 本次實作與變更
- **數據編輯表格功能擴充**：於「數據編輯表格」新增 Alignment 顯示欄位，並於控制面板加入 ALIGNMENT 多選篩選器，支援多條件快速過濾。
- **打包部署同步**：建置生成新版 `index.html` 並推送至 Repo。

### 固定設定值（不在 GUI / CONFIG 中的常數）
- 有效 sheet：`["90", "130", "110", "25", "-45", "90C", "130C", "110C", "25C", "-45C"]`
- 資料欄位起始欄：第 `24` 欄
- Vcc / VIO 參考列：第 `3` / `4` 列
...
### 常見錯誤與解法
- 找不到 `Cout`：檢查每個目標 sheet 第一欄是否包含 `Cout` 定位列。
- Item count 不一致：確認多個來源檔案/溫度 sheet 的 item 結構一致。
